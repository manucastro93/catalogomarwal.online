...
export default function ServicioComercialCompras() {
  ...
  const [personalDuxId, setPersonalDuxId] = createSignal("");
  const [personalDuxLista] = createResource(obtenerPersonalDux);
  const limit = 10;
  ...
  const [evolucionMensual] = createResource(
    () => ({ desde: "2015-01-01", hasta: hasta(), cliente: cliente(), personalDuxId: personalDuxId() }),
    fetchEvolucionEficienciaMensual
  );
  ...
  const [detalleEficiencia] = createResource(
    () => [modo(), desde(), hasta(), categoriaId(), producto(), cliente(), personalDuxId()],
    fetchDetalleActual
  );
  ...
  return (
    <div class="w-full max-w-screen-xl mx-auto px-3 py-4 md:p-6 space-y-6 md:space-y-8">
      ...
      <FiltrosEficiencia
        ...
        personalDuxLista={personalDuxLista() || []}
      />
