...
export default function ModalNuevaPieza(props: {
  ...
  const handleChange = (e: Event) => {
    ...
    const name = target.name as keyof Pieza;
    setForm(f => ({
      ...f,
      [name]: target.value === '' ? undefined : (target.value as any),
    }));
  };
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      if (!form().codigo || !form().categoriaPiezaId || !form().rubroId) {
      ...
      <div class="bg-white p-6 rounded-2xl shadow-xl w-full max-w-xl relative">
        ...
        <form class="grid grid-cols-2 gap-4 mt-2" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <select
              name="categoriaPiezaId"
              value={form().categoriaPiezaId ?? ''}
          ...
          <div>
            ...
            <input
              ...
              readOnly
            />
            <input type="hidden" name="rubroId" value={RUBRO_INYECCION_ID} />
          </div>
