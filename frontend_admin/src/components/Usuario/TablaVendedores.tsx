...
export default function TablaVendedores(props: {
  vendedores: Vendedor[];
  orden: string;
  direccion: "asc" | "desc";
  onOrdenar: (col: string) => void;
  onVer: (v: Vendedor) => void;
  onEditar: (v: Vendedor) => void;
  ...
  return (
    <div class="overflow-auto border rounded-lg">
      <table class="w-full text-sm border-collapse">
        ...
        <tbody>
          <Show when={props.vendedores.length > 0} fallback={
            <tr>
              <td colSpan={5} class="p-4 text-center text-gray-500">No se encontraron vendedores</td>
            </tr>
          }>
              ...
              {(v) => (
                <tr class="border-b hover:bg-gray-50">
                  ...
                  <td class="p-3 flex gap-2">
                    <button class="text-blue-600 hover:underline" onClick={() => props.onVer(v)}>
                      Ver
                    </button>
                    <button class="text-green-600 hover:underline" onClick={() => props.onEditar(v)}>
                      Editar
                    </button>
                    <button class="text-red-600 hover:underline" onClick={() => props.onEliminar(v.id)}>
                      Eliminar
                    </button>
                  </td>
                </tr>
